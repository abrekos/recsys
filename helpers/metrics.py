...
def get_default_metrics(k: int) -> dict[str, MetricAtK]:
    return {
...
def get_default_splitter() -> Splitter:
...
def split_dataset(splitter: Splitter, interactions: Interactions) -> InteractionFolds:
...
def calculate_metrics(
    ...
    if not metrics or len(metrics) == 0:
        metrics = {metric_name: metric for k in [1, 5, 10] for metric_name, metric in get_default_metrics(k).items()}

    splitter = get_default_splitter() if not splitter else splitter
    results = []

    for train_ids, test_ids, fold_info in split_dataset(splitter=splitter, interactions=interactions):
